...
export default function UserPortfolio() {
  ...
  const fetchPortfolio = async () => {
    try {
      ...
      if (err.response?.status === 404) {
        setError("No portfolios found. Create one to get started!");
        setAllPortfolios([]); // clear portfolios
        setTotals(null);      // no totals if none exist
      } else {
  ...
  // --- Loading state ---
  if (!totals && !error && allPortfolios.length === 0) {
    return (
  ...
  // --- Main layout (shared for both error & success) ---
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-background transition-colors duration-400">
      <div className="container mx-auto px-4 sm:px-6 lg:px-12 py-10 space-y-10">
        ...
        {/* Summary Cards (only if totals exist) */}
        {totals && (
        ...
        <div className="min-h-screen bg-gray-100 dark:bg-background transition-colors">
          ...
          <CreatePortfolioForm
            ...
            onPortfolioCreated={(newPortfolio) =>
              setAllPortfolios([...allPortfolios, newPortfolio])
            }
          />
          <HoldingFormModal
